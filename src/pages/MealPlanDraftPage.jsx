...
function MealPlanDraftPage({ auth }) {
  ...
  async function loadDraft() {
    ...
    try {
      ...
      const normalized = parsed
        .map((p) => {
          if (typeof p === 'number') {
            return { id: Number(p), mealType: 'DINNER', servings: 1 }
          }
      ...
      for (const it of uniqueById) {
        try {
          ...
          resolved.push({
            id: it.id,
            title: data?.title || data?.name || `Recipe ${it.id}`,
  ...
  function clearDraft() {
    ...
    setItems([])
    setNotification({
      message: 'Cleared draft mealplan.',
      type: 'success',
    })
  }
  ...
  async function handleCreateFromDraft() {
    if (!items || items.length === 0) {
      setNotification({
        message: 'No recipes in draft to create a mealplan.',
        type: 'info',
      })
      return
    }
    if (!mealplanName || mealplanName.trim().length === 0) {
      setNotification({
        message: 'Please provide a name for the mealplan.',
        type: 'info',
      })
      return
    }
    ...
    const payload = {
      ...
      listOfEntries: items.map((it) => ({
        recipeId: Number(it.id),
        servingsOverride: Number(it.servings) || 1,
    ...
    try {
      ...
      setItems([])
      setNotification({
        message: `Mealplan "${created?.name || created?.id}" created.`,
        type: 'success',
      })
      if (created && created.id) {
        navigate(`/mealplans/${created.id}/view`)
      } else {
        navigate('/mealplans')
      }
    } catch (err) {
      setNotification({
        message: `Failed to create mealplan: ${err.message}`,
        type: 'error',
      })
    } finally {
      setCreating(false)
    ...
    <>
      ...
      <section className="card">
        ...
        <div style={{ marginBottom: '0.75rem' }}>
          ...
          <button
            className="btn"
            style={{ marginLeft: '0.5rem', border: '1px solid #4b5563' }}
            onClick={clearDraft}
          >
            Clear draft
          </button>
        ...
        {!loading && items.length === 0 && (
          <p className="muted">
            No recipes in draft. Add recipes from the recipes list.
          </p>
        )}
            ...
            {items.map((it) => (
              ...
              >
                <div>
                  ...
                  <div className="muted">
                    ID: {it.id} · Category: {it.mealType} · Servings:{' '}
                    {it.servings} (base: {it.baseServings})
                  </div>
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  >
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setItems((prev) =>
                          prev.map((p) =>
                            p.id === it.id ? { ...p, dayOfWeek: newDay } : p,
                          ),
                        )
                        const raw = localStorage.getItem('mealplan_draft_ids')
                        const parsed = raw ? JSON.parse(raw) : []
                  ...
                  <div className="muted">
                    ...
                    {it.baseServings
                      ? Math.round(
                          (it.servings / it.baseServings) * 100,
                        ) / 100
                      : 1}
                    ×
                  </div>
